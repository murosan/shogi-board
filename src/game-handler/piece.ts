...
export default class PieceObj {
  ...
  constructor(name: string, whose: number, row: number, col: number, _movs?: Array<PorE>) {
    this.name = name;
    this.whose = whose;
    this.row = row;
    this.col = col;
  ...
  canPromote(r: number): boolean {
    ...
    const inEnemyArea: () => boolean = (() => {
      return ((whose === 0) && (r <= 2 || row <= 2)) ||
        ((whose === 1) && (6 <= r || 6 <= row));
    });
    if (row === -1) {
      return false;
    } else if ((name === '飛' || name === '角' || name === '銀' ||
      name === '桂' || name === '香' || name === '歩') && inEnemyArea()) {
      return true;
    } else {
      return false;
    }
