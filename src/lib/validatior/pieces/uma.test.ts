...
describe('馬の動き判定', async () => {
  it('障害物がないとき、角と玉を合わせた動きができる', async () => {
    ...
    const res1: Point[] = uma(pos, { row: 4, column: 4, piece: Uma0 })
    const res2: Point[] = uma(pos, { row: 4, column: 4, piece: Uma1 })
  ...
  it('味方の駒があるところには移動できない(先手)', async () => {
    ...
    const res: Point[] = uma(pos, { row: 4, column: 4, piece: Uma0 })
  ...
  it('味方の駒があるところには移動できない(後手)', async () => {
    ...
    const res: Point[] = uma(pos, { row: 4, column: 4, piece: Uma1 })
    expect(new Set(res)).toEqual(expected)
  })
